...
public class ConcurrentRMA6Processor implements Callable<RMA6Processor>{
	...
	private  boolean wantMeganSummaries = false;
	  ...
	  public ConcurrentRMA6Processor(String inDir, String fileName, String outDir, NCBI_MapReader mapReader, NCBI_TreeReader treeReader, List<Integer> taxIDs, double topPercent, int i, double minPI, Filter b, Taxas t, boolean verbose, Logger log, Logger warning, boolean reads, double minCompl, boolean alignments, boolean wantMeganSummaries, boolean turnOffDestacking)
	  {
	    this.inDir = inDir;
	    this.outDir = outDir;
	  ...
	  public RMA6Processor call() {
	    RMA6Processor processor = new RMA6Processor(inDir, fileName, outDir, mapReader, 
	      treeReader, maxLength, minPIdent, behave, t, verbose, log, warning, wantReads, minComplexity, wantAlignments, wantMeganSummaries, turnOffDestacking);
	    processor.process(taxIDs, topPercent);
	    return processor;
	  }
