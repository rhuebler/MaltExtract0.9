...
public class NodeProcessor{
		...
		//constructors
		public NodeProcessor(int id, double minPIdent, NCBI_MapReader reader, boolean v, Logger log, Logger warning, boolean reads, Filter behave, double minCompl, boolean alignment, boolean turnOffDestacking)
		  {
		    taxID = Integer.valueOf(id);
		...
		public void process(String inDir, String fileName, double topPercent, int maxLength){ 
			  if (behave == Filter.ANCIENT) {
			      ancientProcessor = new ExperimentalRMA6AncientDestacker(taxID.intValue(), minPIdent, mapReader, verbose, log, log, wantReads, topPercent, maxLength, alignment, turnOffDestacking, behave);
			    } else if (behave == Filter.NON) {
			      defaultProcessor = new ExperimentalRMA6Destacker(taxID.intValue(), minPIdent, mapReader, verbose, log, log, wantReads, topPercent, maxLength, alignment, turnOffDestacking, behave);
