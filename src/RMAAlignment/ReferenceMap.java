...
public class ReferenceMap {
	...
	private void calculate (ArrayList<Alignment> alignments, double averageCoverage){
		...
		for(int key: coverageContainer.keySet()) {
			  averageCoverage += coverageContainer.get(key);
		}
		averageCoverage /= length;
	    this.averageCoverage = averageCoverage;
	    if (averageCoverage >= 10.0D) {
	      turnedOn = false;
	    }
		while(i<alignments.size()){
		//calculate unique positions per read plus average distance between current and next read
