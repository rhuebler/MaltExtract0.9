...
public class CompositionMap {
	public CompositionMap(HashMap<Integer,ArrayList<Alignment>> map){
	 ...
	 public CompositionMap(HashMap<Integer, ArrayList<Alignment>> map, boolean turnOffDestacking) { setCompositionMap(map);
...
public HashMap<Double, ArrayList<Integer>> getAllTopReferences() {
    HashMap<Double, ArrayList<Integer>> additional = new HashMap<>();
      ...
      for (double x = 1.0; x >= 0.0; x -= 0.1) {
        ...
        for (int key : map.keySet()) {
          if ((map.get(key).size() > maxSize * x) && (map.get(key).size() <= maxSize * (x + 0.1))) {
            margin.add(Integer.valueOf(key));
          }
        }
        additional.put(Double.valueOf(x), margin);
      }
    }
    return additional;
  }
